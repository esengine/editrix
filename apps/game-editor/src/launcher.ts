...
const ICONS: Record<string, string> = {
  ...
  'file-text': svg('<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/>'),
  ...
  'folder-open': svg('<path d="M5 19a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2h4l2 2h4a2 2 0 0 1 2 2v1"/><path d="M3.5 15h17l-2.5-7H6z"/>'),
  ...
  'share-2': svg('<circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/>'),
  ...
  // App icon
  'app-icon': svg('<circle cx="12" cy="12" r="3"/><path d="M12 2v4"/><path d="M12 18v4"/><path d="M4.93 4.93l2.83 2.83"/><path d="M16.24 16.24l2.83 2.83"/><path d="M2 12h4"/><path d="M18 12h4"/><path d="M4.93 19.07l2.83-2.83"/><path d="M16.24 7.76l2.83-2.83"/>'),
};
...
type NavId = 'projects' | 'templates' | 'installs' | 'assets' | 'learn' | 'community' | 'donate' | 'new-project';
...
const TEMPLATES: TemplateEntry[] = [
  { id: 'empty', name: 'Empty Project', description: 'A blank project with no pre-configured scenes or assets', icon: 'file-text' },
  { id: '2d-game', name: '2D Game', description: 'Basic 2D setup with sprite rendering and camera', icon: 'grid' },
  { id: '3d-game', name: '3D Game', description: '3D scene with PBR lighting, camera controls, and sample model', icon: 'box' },
  { id: 'ui-app', name: 'UI Application', description: 'App template with panels, forms, and navigation', icon: 'layout' },
  { id: 'node-editor', name: 'Node Editor', description: 'Visual scripting / node graph editor template', icon: 'share-2' },
];
...
function renderProjectList(container: HTMLElement): void {
  ...
  const filtered = projects.filter((p) =>
    !filterText || p.name.toLowerCase().includes(filterText) || p.path.toLowerCase().includes(filterText),
  );
...
function renderNewProject(main: HTMLElement): void {
  ...
  for (const tpl of TEMPLATES) {
    const card = el('div', `el-np-tpl-card${tpl.id === selectedTemplate ? ' el-np-tpl-card--selected' : ''}`);
