      ...
      view.registerFactory('viewport', (id) => {
        ...
        const refreshPrefabBanner = (): void => {
          ...
          if (activePath?.endsWith('.esprefab') === true) {
            ...
            widget?.setPrefabBanner({
              ...
              onExit: () => {
                documentService.close(activePath);
              },
            });
          } else {
        ...
        const refreshAnimation = (): void => {
          ...
          if (activePath?.endsWith('.esanim') === true) {
            animWidget ??= new AnimationEditorWidget(
              `${id}-anim`,
              animationService,
              catalog,
              project,
            );
            animWidget.setOnExit(() => {
              documentService.close(activePath);
            });
        ...
        ctx.subscriptions.add(
          widget.onDidDropAsset(({ absolutePath, worldX, worldY, hitEntityId }) => {
            ...
            if (entry.type === 'scene') {
              documentService.open(absolutePath).catch(() => {
                /* doc-sync surfaces failure */
              });
              return;
            }
            if (entry.type === 'prefab') {
              ...
              void (async (): Promise<void> => {
                try {
                  const rootEntityId = await prefabService.instantiate(entry.uuid, {
                  ...
                  undoRedo.push({
                    ...
                    redo: () => {
                      void prefabService
                        .instantiate(entry.uuid, { position: { x: worldX, y: worldY } })
                        .then((newRootId) => {
            ...
            if (hitEntityId !== undefined && ecs.hasComponent(hitEntityId, 'Sprite')) {
              ...
              const newId = createSpriteEntity(
                ecs,
                undoRedo,
                entry.uuid,
                entry.relativePath,
...
function replaceSpriteTexture(
  ...
  undoRedo.push({
    ...
    undo: () => {
      ecs.setEntityMetadata(entityId, ASSET_METADATA_KEY, prevUuid);
    },
    redo: () => {
      ecs.setEntityMetadata(entityId, ASSET_METADATA_KEY, newUuid);
    },
