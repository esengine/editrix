...
export const PrefabPlugin: IPlugin = {
  ...
  activate(ctx: IPluginContext) {
    ...
    const isInsideInstance = (entityId: number): boolean =>
      findInstanceRoot(entityId) !== undefined;
    ...
    const buildPrefabData = (rootEntityId: number, name: string): PrefabData => {
      ...
      const build = (ecsId: number, parentEcsId: number | null): void => {
        ...
        const parentPrefabId =
          parentEcsId !== null ? (ecsToPrefabId.get(parentEcsId) ?? null) : null;
    ...
    const matchesRef = (override: PrefabOverride, ref: PrefabOverrideRef): boolean => {
      ...
      if (ref.propertyName !== undefined && override.propertyName !== ref.propertyName)
        return false;
    ...
    ctx.subscriptions.add(
      documentService.registerHandler({
        ...
        async load(filePath, content): Promise<void> {
          ...
          const { entities, rootId: rootPlaceholder } = flattenWithPlaceholderIds(
            prefabData,
            graph,
          );
