...
export const PrefabPlugin: IPlugin = {
    descriptor: {
        id: 'app.prefab',
        version: '1.0.0',
        dependencies: ['app.ecs-scene', 'app.filesystem', 'app.project', 'app.asset-catalog', 'app.play-mode', 'app.document-sync'],
    },
    activate(ctx: IPluginContext) {
        ...
        ctx.subscriptions.add(onDidCreateInstance);
        ctx.subscriptions.add(onDidHotReload);

        /** Cache of loaded+migrated prefab data keyed by source UUID. */
        const prefabCache = new Map<string, PrefabData>();

        const sourceCacheInvalidate = (uuid: string): void => { prefabCache.delete(uuid); };
        ...
        const getInstanceInfo = (entityId: number): PrefabInstanceInfo | undefined => {
            ...
            const overrides = (ecs.getEntityMetadata(rootId, PREFAB_METADATA_KEYS.OVERRIDES) as PrefabOverride[] | undefined) ?? [];
            ...
            const nonPlacementCount = rootPidStr !== undefined
        ...
        const extractInstanceState = (rootEntityId: number): ProcessedEntity[] => {
            ...
            const walk = (ecsId: number, parentEcsId: number | null): void => {
                ...
                const components = ecs.getComponents(ecsId)
        ...
        const createPrefab = async (entityId: number, filePath: string): Promise<string> => {
            ...
            if (isInstanceRoot(entityId) || isInsideInstance(entityId)) {
                throw new Error('Cannot create a prefab from an entity that is already part of a prefab instance.');
            }
        ...
        const materializeFlattened = (
            ...
            for (const e of ordered) {
                const parentEcs = e.id === rootPlaceholder
                    ? instanceParent
                    : (e.parent !== null ? idMap.get(e.parent) : undefined);
        ...
        const instantiate = async (
            ...
            const { rootEntityId } = materializeFlattened(ecs, entities, rootId, options?.parent, sourceUuid);
        ...
        const flushOverrides = (): void => {
            if (debounceHandle) { clearTimeout(debounceHandle); debounceHandle = undefined; }
         ...
         */
        const structuralReload = (rootId: number, newSource: PrefabData, graph?: ReadonlyMap<string, PrefabData>): void => {
            const ecs = getEcs();
            if (!ecs) return;
            ...
            try {
                ...
                for (const [pid, ecsId] of oldByPrefabId) {
                    ...
                    const remappedComponents = pe.components.map(c => ({ type: c.type, data: { ...c.data } }));
                    for (const comp of remappedComponents) {
                        ...
                        for (const field of schema) {
                            ...
                            if (!target) { comp.data[field.key] = 0; continue; }
        ...
        const fullReinstantiate = (
            ...
            const sourceUuid = ecs.getEntityMetadata(rootId, PREFAB_METADATA_KEYS.SOURCE) as string | undefined;
            if (typeof sourceUuid !== 'string') return;
            ...
            const { rootEntityId: newRoot } = materializeFlattened(ecs, entities, placeholder, parent ?? undefined, sourceUuid);
        ...
        const wireMutationWatchers = (ecs: IECSSceneService): void => {
            ctx.subscriptions.add(ecs.onPropertyChanged(ev => { markInstanceDirty(ev.entityId); }));
            ctx.subscriptions.add(ecs.onComponentAdded(ev => { markInstanceDirty(ev.entityId); }));
            ctx.subscriptions.add(ecs.onComponentRemoved(ev => { markInstanceDirty(ev.entityId); }));
            ctx.subscriptions.add(ecs.onVisibilityChanged(ev => { markInstanceDirty(ev.entityId); }));
            ...
            ctx.subscriptions.add(ecs.onHierarchyChanged(() => {
                ...
                if (roots.size > 0) {
                    if (debounceHandle) clearTimeout(debounceHandle);
                    debounceHandle = setTimeout(() => { debounceHandle = undefined; flushOverrides(); }, OVERRIDE_DEBOUNCE_MS);
                }
            }));
        };
        ...
        if (presence.current) {
            ...
            ctx.subscriptions.add(presence.onDidBind(ecs => { wireMutationWatchers(ecs); }));
        }

        // Catalog changes → hot reload
        ctx.subscriptions.add(catalog.onDidChange(change => {
        ...
        ctx.subscriptions.add(playMode.onDidChangeMode(ev => {
        ...
        const isComponentOverridden = (entityId: number, componentType: string): boolean => {
            for (const o of getOverridesForEntity(entityId)) {
                if ((o.type === 'component_added' || o.type === 'component_replaced' || o.type === 'component_removed')
        ...
        const isMetadataOverridden = (entityId: number, metadataKey: string): boolean => {
            for (const o of getOverridesForEntity(entityId)) {
                if ((o.type === 'metadata_set' || o.type === 'metadata_removed') && o.metadataKey === metadataKey) {
                    return true;
                }
            }
            return false;
        ...
        const revertPropertyOverride = (entityId: number, componentType: string, fieldPath: string): void => {
            const ecs = getEcs();
            if (!ecs) return;
            const pid = ecs.getEntityMetadata(entityId, PREFAB_METADATA_KEYS.ENTITY_ID);
            if (typeof pid !== 'string') return;
        ...
        const countInstancesOf = (sourceUuid: string): number => findAllInstanceRootsOf(sourceUuid).length;
        ...
        const createVariant = async (baseUuid: string, filePath: string): Promise<string> => {
            ...
            const metaPath = `${filePath}.meta`;
            await fileSystem.writeFile(metaPath, JSON.stringify({ uuid, version: 1, importer: {} }, null, 2) + '\n');
        ...
        ctx.subscriptions.add(documentService.onDidChangeActive((newPath) => {
            if (!isEcsOccupyingDoc(newPath)) return;
            if (newPath === currentEcsDocPath) return;
            swapEcsTo(newPath);
        }));
        ...
        const processedEntitiesToSceneData = (
            flat: readonly ProcessedEntity[],
            name: string,
        ): { version: number; name: string; entities: {
            id: number; name: string; children: number[];
            components: Record<string, Record<string, unknown>>;
            visible?: boolean; metadata?: Record<string, unknown>;
        }[] } => ({
            version: 1,
            name,
            entities: flat.map(e => ({
        ...
        ctx.subscriptions.add(documentService.registerHandler({
            extensions: ['.esprefab'],
            async load(filePath, content): Promise<void> {
                ...
                const rel = project.isOpen
                    ? (filePath.startsWith(project.path + '/') ? filePath.slice(project.path.length + 1) : undefined)
                    : undefined;
                ...
                const graph = sourceUuid !== undefined
                    ? await preloadPrefabGraph(sourceUuid)
                    : undefined;
        ...
        ctx.subscriptions.add(documentService.onDidChangeDocuments(() => {
            ...
            const stillOpen = documentService.getOpenDocuments()
        ...
        ctx.subscriptions.add(presence.onDidBind(() => {
            ...
            void documentService.open(filePath).catch(() => { /* already logged */ });
        ...
        const service: IPrefabService = {
            ...
            adoptEcsDoc: (filePath: string): void => { currentEcsDocPath = filePath; },
        ...
        // Suppress unused-var warnings for helpers referenced in later steps.
        void project;
    },
};
