...
import type {
  ...
  IAssetRevealService as IAssetRevealServiceShape,
  IECSScenePresence as IECSScenePresenceShape,
  IInspectorComponentFilter as IInspectorComponentFilterShape,
  IPrefabService as IPrefabServiceShape,
  ISharedRenderContext as ISharedRenderContextShape,
  PrefabInstanceInfo,
} from '../services.js';
import { IAssetRevealService, parseSelectionRef } from '../services.js';
...
function ecsToPropertyGroups(
  ...
  entityId: number,
  filter: IInspectorComponentFilterShape,
...
export interface InspectorPanelDeps {
  ...
  readonly presence: IECSScenePresenceShape;
  readonly renderContextSvc: ISharedRenderContextShape;
...
export function setupInspectorPanel(deps: InspectorPanelDeps): void {
  const {
    layout,
    view,
    selection,
  ...
  // which activates after the inspector. Resolved on demand inside the
  // click handler (by which point it exists).
  const tryReveal = (): IAssetRevealServiceShape | undefined => {
    try {
      return services.get(IAssetRevealService);
    } catch {
      return undefined;
    }
  };
  ...
  const logError = (msg: string, source = 'inspector'): void => {
    try {
      services.get(IConsoleService).log('error', msg, source);
    } catch {
  ...
  const updatePrefabHeader = (info: PrefabInstanceInfo | undefined): void => {
    if (!prefabHeaderEl || !prefabHeaderTitleEl || !prefabHeaderRevertBtn || !prefabHeaderApplyBtn)
      return;
    if (!info) {
  ...
  subscriptions.add(
    presence.onDidBind((ecs) => {
      subscriptions.add(ecs.onPropertyChanged(refreshInspector));
      subscriptions.add(ecs.onComponentAdded(refreshInspector));
      subscriptions.add(ecs.onComponentRemoved(refreshInspector));
      subscriptions.add(
