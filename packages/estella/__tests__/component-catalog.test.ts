...
import { ComponentCatalog, type SdkComponentDef, type SdkComponentInfo } from '../src/component-catalog';
...
function makeInfo(name: string, defaults: Record<string, unknown> = {}): SdkComponentInfo {
    return { name, def: makeDef(name, defaults), defaults, isTag: Object.keys(defaults).length === 0 };
}
