  ...
  activate(ctx: IPluginContext) {
    ...
    ctx.subscriptions.add(
      settings.registerGroup({
        ...
        settings: [
          {
            key: 'editrix.console.maxEntries',
            label: 'Max Log Entries',
            type: 'number',
            defaultValue: 1000,
            description: 'Maximum number of log entries to keep before oldest are removed',
          },
          {
            key: 'editrix.console.showTimestamp',
            label: 'Show Timestamps',
            type: 'boolean',
            defaultValue: true,
            description: 'Display timestamps next to log entries',
          },
          {
            key: 'editrix.console.fontSize',
            label: 'Font Size',
            type: 'range',
            defaultValue: 12,
            min: 10,
            max: 20,
            step: 1,
            description: 'Font size for console log entries',
          },
          {
            key: 'editrix.console.captureKernelEvents',
            label: 'Capture Kernel Events',
            type: 'boolean',
            defaultValue: true,
            description: 'Automatically log kernel events (plugin activation etc.)',
          },
        ],
      }),
    );
